...
import { carregarLojas, salvarLojas, proximoIdLoja, type Loja } from "./data/lojas";
import {
    listarFornecedores,
...
r.post("/extrair-html", (req, res) => {
    try {
        ...
        if (!html.trim()) {
            res.status(400).json({ ok: false, erro: "HTML vazio" });
            return;
        }
        const extraido = parseNfceHtml(html);
        res.json({ ok: true, produtos: extraido.produtos, emitente: extraido.emitente });
    } catch (err) {
...
r.post("/lojas", async (req, res) => {
    try {
        ...
        if (!body.CNPJ || !body.xNome) {
            res.status(400).json({ ok: false, erro: "CNPJ e nome obrigatórios" });
            return;
        }
...
r.get("/fornecedores/:cnpj", async (req, res) => {
    ...
    if (!f) { res.status(404).json({ ok: false, erro: "Não encontrado" }); return; }
...
r.post("/fornecedores", async (req, res) => {
    try {
        const body = req.body;
        if (!body.CNPJ) { res.status(400).json({ ok: false, erro: "CNPJ obrigatório" }); return; }
...
r.post("/ean/aprender", async (req, res) => {
    try {
        ...
        if (!ean || !/^\d{8}$/.test(NCM)) {
            res.status(400).json({ ok: false, erro: "EAN e NCM (8 dígitos) são obrigatórios." });
            return;
        }
