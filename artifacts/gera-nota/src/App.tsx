...
export default function App() {
  ...
  const [acField, setAcField] = useState<'nome' | 'cnpj' | 'fechado'>('nome');
  ...
  const suggestions = (acField === 'nome'
    ? fornecedores.filter(f => f.xNome.toLowerCase().includes(nomeQuery))
    : fornecedores.filter(f => f.CNPJ.startsWith(cnpjQuery))).slice(0, 6);
  return <div>
